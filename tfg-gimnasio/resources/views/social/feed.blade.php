...
@section('content')
    <div class="container mx-auto px-4 py-8 max-w-2xl">
        ...
        {{-- MODAL: Formulario para crear publicación --}}
        <div id="createPostModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-lg bg-white">
                ...
                <form action="{{ route('post.create') }}" method="POST">
                    ...
                    {{-- Datos del ejercicio (opcionales) --}}
                ...
                <div class="bg-white rounded-xl shadow-md mb-6 overflow-hidden hover:shadow-lg transition">
                    ...
                    <div class="p-4 border-b">
                        <div class="flex items-center">
                            {{-- Verificar que el usuario existe antes de mostrar sus datos --}}
                            @if ($post['user'])
                            ...
                            @else
                                {{-- Si el usuario no existe, mostrar datos por defecto --}}
                                <div
                                    class="w-12 h-12 rounded-full bg-gray-300 mr-3 flex items-center justify-center text-gray-500">
                                    ?
                                </div>
                                <div>
                                    <span class="font-semibold text-gray-500">
                                        Usuario desconocido (ID: {{ $post['user_id'] }})
                                    </span>
                                    <p class="text-xs text-gray-400">
                                        {{ \Carbon\Carbon::parse($post['created_at'])->diffForHumans() }}
                                    </p>
    ...
    <div class="p-4">
        ...
        {{-- Datos del ejercicio (si existen) --}}
